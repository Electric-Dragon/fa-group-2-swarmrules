...
from src.entities.agent import BaseAgent, PheromoneAgent
...
def main():
    ...
    env.generate_hybrid_environment(seed=42)

    # 2. Initialize simulation
    sim = Simulation(env, dt=0.01, seed=123)

    # 3. Add an attractor in the center
    center = np.array([grid_size * 0.5, grid_size * 0.5])
    sim.add_object(Attractor(obj_id=0, position=center, strength=2.0, cutoff=12.0))
